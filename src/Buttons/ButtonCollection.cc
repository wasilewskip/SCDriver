...
std::vector<ButtonDataChangedEvent> ButtonCollection::update(const SteamInputPacket& packet)
{
    std::vector<ButtonDataChangedEvent> events;
    for(const auto& button : buttons)
    {
        if(auto event = button->updateState(packet))
        {
